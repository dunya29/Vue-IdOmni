...
const fetchDocs = async () => {
    ...
    try {
        ...
        sections.value = sections.value.map((sec) => {
            return {
                ...sec,
                docs: data.filter((doc) => doc.sectionId === sec.id),
            };
        });
    } catch (err) {
        console.log(err);
...
const sortDocs = async (thisItem, changedItemsArr) => {
    ...
    try {
        await docsApi.sortDocs(routeId.value, thisItem.id, {
...
onMounted(async () => {
    ...
    if (storeAuth.userData.userRole === "admin") {
        pageLoad.value = true;
        fetchSections();
    } else {
        let urlMatchInvite = route.fullPath.match("invite");
        let link = urlMatchInvite
            ? route.fullPath.substring(urlMatchInvite.index + 7)
            : "";
        if (storeAuth.logged) {
            const login = storeAuth.userData.login;
            const email = storeAuth.userData.email;
            try {
                const { data } = await inviteApi.checkUser(
                    routeId.value,
                    email
                );
                if (data.length > 0) {
                    ...
                    if (urlMatchInvite || localStorage.getItem("inviteLink")) {
                        try {
                            let pageInviteLink = link
                                ? link
                                : JSON.parse(localStorage.getItem("inviteLink"))
                                      .link;
                            const { data } = await inviteApi.checkInviteLink(
                                routeId.value,
                                pageInviteLink
                            );
                            if (data.length > 0) {
            ...
            if (urlMatchInvite) {
                localStorage.setItem(
                    "inviteLink",
                    JSON.stringify({ id: routeId.value, link })
                );
            }
        ...
        <section class="section intro docs-p" v-if="pageLoad">
            <div class="container">
                <div class="sec-top docs-p__top">
                    ...
                    <Invite v-if="storeAuth.userData.userRole === 'admin'" />
                </div>
                <div class="docs-p__items" v-if="sectionTotal" :class="docsLoading && 'loading'">
                    <Section v-for="item in sections" :key="item.id" :id="item.id" :title="item.title" :item="item.docs" :userRole="storeAuth.userData.userRole" @sortDocs="sortDocs" @delSec="delSec" @changeSecName="changeSecName" />
                </div>
                <AddSection ref="addSecForm" v-if="storeAuth.userData.userRole === 'admin'" :addSecLoading="addSecLoading" @addNewSec="addNewSec" />
            </div>
        </section>
    </PageWrap>
</template>
